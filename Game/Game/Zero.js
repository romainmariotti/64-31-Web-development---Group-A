...
export const engineSoundZero = new Audio("../Game/Sound/EngineZero.mp3");
...
export function fireZeroBullet() {
  ...
  const noseX_right = Zero.x + Zero.width * 0.7;
  ...
  zeroBullets.push(
    {
      x: noseX_right,
      y: noseY,
      width: zeroBulletConfig.width,
      height: zeroBulletConfig.height,
    },
    {
      x: noseX_left,
      y: noseY,
...
export const zeroShootingSound = new Audio("../Game/Sound/20mmZero.mp3");
...
secondaryShootingSound.onerror = () =>
  console.error("Failed to load SecondaryFire.mp3");
  ...
  zeroSecondaryBullets.push(
    {
      x: noseX_right,
      y: noseY,
      width: zeroSecondaryBulletConfig.width,
      height: zeroSecondaryBulletConfig.height,
    },
    {
      x: noseX_left,
      y: noseY,
...
document.addEventListener("mousedown", (event) => {
  if (
    event.button === 2 &&
    activeJet &&
    activeJet.image.src.includes("A6MZero")
  ) {
...
export function zeroActions() {
  ...
  if (
    (zeroKeys["ArrowRight"] || zeroKeys["d"]) &&
    Zero.x + Zero.width < canvas.width
  ) {
    Zero.x += 20;
  }
  ...
  if (
    (zeroKeys["ArrowDown"] || zeroKeys["s"]) &&
    Zero.y + Zero.height < canvas.height
  ) {
    Zero.y += 20;
  }
